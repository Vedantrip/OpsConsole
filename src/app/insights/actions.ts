...
export async function extractInsightMetrics(rawData: any) {
  ...
  return {
    ...
    followingCount: parseNum(profile.following),
    postCount: parseNum(profile.posts),
    ...
    overallScore: parseNum(scores.overall),
    engagementScore: parseNum(scores.engagement),
    audienceScore: parseNum(scores.audience),
    contentQualityScore: parseNum(scores.content),
    consistencyScore: parseNum(scores.consistency),
    audienceData,
    topLocations: audienceData.locations,
    interests: audienceData.interests,
    ageDistribution: audienceData.age,
    genderDistribution: audienceData.gender,
    source: audienceData.source,
    sourceVersion: audience.sourceVersion ?? scores.methodology ?? null,
  };
}
...
export async function saveInsightSnapshot(handle: string, rawData: any) {
  ...
  const creators = await prisma.creator.findMany({
    where: { ...creatorScope(context), handle: { not: null } },
  });
...
async function createInsightSnapshot(creatorId: string, rawData: any) {
  ...
  const insight = await prisma.creatorInsight.create({
    data: {
      ...
      audienceData: metrics.audienceData,
