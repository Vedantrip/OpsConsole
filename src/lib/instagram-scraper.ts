...
export async function scrapeInstagramData(
  ...
  if (RAPIDAPI_KEY) {
    try {
      ...
      if (data && data.reels.length > 0) return data;
    } catch (err: any) {
...
/**
 * Provider 1: RapidAPI Scraper
 */
async function scrapeViaRapidApi(username: string, limit: number): Promise<ScrapedProfile> {
  const url = `https://${RAPIDAPI_HOST}/userinfo/${encodeURIComponent(username)}`;
...
/**
 * Provider 2: ScraperAPI (Direct proxy to Instagram Web Profile JSON)
 */
async function scrapeViaScraperApi(username: string, limit: number): Promise<ScrapedProfile> {
  ...
  const proxyUrl = `https://api.scraperapi.com?api_key=${encodeURIComponent(
    SCRAPERAPI_KEY
  )}&url=${encodeURIComponent(targetUrl)}&keep_headers=true`;
